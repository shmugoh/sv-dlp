...
class urls:
    ...
    def _cbk_to_dict(url: str) -> dict:
        '''
        Turn CBK to Python Dictionary format, as
        the CBK call is outputed in a XML format.

        Automatically a GET request is performed, so no need
        to parse one.
        '''
...
if __name__ == "__main__":
    print(urls._build_pano_url(50.95475387573242,6.971647262573242))
    pano_id = "jYxwHUdPuhm8NGfAH6y8IA"
    max_zoom = _find_max_zoom(pano_id)
    half_zoom = max_zoom // 2
    print(urls._build_tile_url(pano_id, half_zoom))
